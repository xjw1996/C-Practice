...
private:
    ...
    std::string outpath = "D:\\xue_workspace\\DataOfUniversityScore\\文科二批大学\\";
...
void Dataset_classification::readfile() {
    ...
    for (int i = 0;i < Major_arr.size(); i++) {
        ...
        out.open(outpath + variable_path + "文科二本2021年在吉专业分数及位次" + ".csv", std::ios::out | std::ios::binary);
        if (out.fail()) {
        ...
        for (int j = 0;j < Data_arr.size();j++) {
            ...
            /*int Serach_ = tem_l.MajorNote.find(Major_arr[i]);
            if (Serach_ == std::string::npos) {
                Search_MajorNote = false;
            }
            else
            {
                Search_MajorNote = true;
            }

            if (Search_MajorNote == true && tem_l.Major != Major_arr[i]) {
                data_2Darr[i].push_back(tem_l);
            }*/
