...
public:
    void readfile();
    void read2020Excel();
    void classification_teacher_and_workwithforengier();
...
private:
    ...
    std::string outpath = "D:\\xue_workspace\\DataOfUniversityScore\\ExcelPythonTest\\专业\\师范类与中外合作文科二批\\";
    std::string variable_path;
    std::vector<SpecialtyList> Data_arr;
    std::vector<SpecialtyList> Data_arr_1;
    ...
    std::vector<std::string> two_Major_arr = { "师范类","中外合作" };
    ...
    std::vector<std::vector<SpecialtyList>> twomajor;
};
...
void Dataset_classification::classification_teacher_and_workwithforengier() {
    char del = ',';
    std::ifstream file;
    file.open(path);
    if (!file.is_open())
    ...
    data_2Darr.resize(2);//使用二维vector数组  必须得对数组的大小进行定义
    for (int i = 0;i < two_Major_arr.size();i++) {
        for (auto it = Data_arr.begin(); it != Data_arr.end(); ++it) {
            int Serach_ = it->MajorNote.find(two_Major_arr[i]);
            if (Serach_ != std::string::npos) {
                //std::cout << it->College<< it->Major <<"专业为" << it->MajorNote << std::endl;
                data_2Darr[i].push_back(*it);
            }
        }
       
        int arr_length = data_2Darr[i].size();
        data_2Darr[i] = BubbleSort(data_2Darr[i], arr_length);
    }
    for (int i = 0;i < two_Major_arr.size(); i++) {
        out.open(outpath + two_Major_arr[i] + "2021年在吉录取分数及位次" + ".csv", std::ios::out | std::ios::binary);
        if (out.fail()) {
            std::cerr << "fail" << std::endl;
            return;
        }
        for (int j = 0;j < data_2Darr[i].size();j++) {
            out << this->data_2Darr[i][j].CollegeCode << "," << this->data_2Darr[i][j].College << "," << this->data_2Darr[i][j].Note << ","
                << this->data_2Darr[i][j].Major << "," << this->data_2Darr[i][j].MajorNote << "," << this->data_2Darr[i][j].LowestScore << ","
                << this->data_2Darr[i][j].LowestPosition << "," << this->data_2Darr[i][j].NumberOfAdmissions << "," << this->data_2Darr[i][j].Characteristic << ","
                << this->data_2Darr[i][j].Level << "," << this->data_2Darr[i][j].Type << std::endl;
        }
        std::cout << "Open file successfully" << std::endl;
        out.close();
    }
...
int main()
{
    ...
    Data_cla.classification_teacher_and_workwithforengier();
}
