...
public:
    ...
    std::string path_2020 = "D:\\xue_workspace\\DataOfUniversityScore\\ExcelPythonTest\\位次差计算以及省份城市追加\\理科二本\\522\\2020投档线位次备注.csv";
    //void classification();
...
private:
    ...
    std::ofstream out;
    std::string path = "D:\\xue_workspace\\DataOfUniversityScore\\ExcelPythonTest\\位次差计算以及省份城市追加\\理科二本\\522\\20212019位次差数据复制.csv";
    //std::string path_ = "C:\\Users\\jinue\\Desktop\\数据\\20200428\\文科二本\\前半部分.csv";
    std::string outpath = "D:\\xue_workspace\\DataOfUniversityScore\\ExcelPythonTest\\位次差计算以及省份城市追加\\理科二本\\522\\0522_2021-2019理科院校录取位次及位次差\\";
...
std::vector<Science> Dataset_classification::BubbleSort(std::vector<Science> A, int n) {
    for (int i = n - 1; i > 0; i--)
        for (int j = 0; j < i; j++)
            if (A[j].Weici2020 > A[j + 1].Weici2020 || A[j + 1].Weici2020 == " ") {
                std::swap(A[j], A[j + 1]);
            }
...
void Dataset_classification::readexcel() {
    ...
    while (std::getline(file2, LineOfData_))
    {
        ...
        /*tem_Science.Isit985 = tem_result2[10];
        tem_Science.Isit211 = tem_result2[11];
        tem_Science.IsitDoubleTop = tem_result2[12];*/
    ...
    out.open(outpath + "2021-2020理科二本院校录取位次及位次差.csv", std::ios::out | std::ios::binary);
    if (out.fail()) {
